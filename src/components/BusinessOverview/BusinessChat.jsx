...
import { StageMobile } from "../Roadmap/StagesMobile";
...
export default function BusinessChat({
  ...
  function formatTaskText(rawText) {
    const formatted = rawText
      ...
      .replace(
        /\*/g,
        '<span style="font-size: 25px; color: #49b8bf;"><b>&#8226;</b></span>',
      )
      .replace(/\n/g, "<br>")
      .trim();
    return formatted;
  ...
  useEffect(() => {
    if (type === "business-overview") {
      setCurrentSocket(
        socket_values.find((socket) => socket.value === "business-overview"),
      );
    } else if (type === "business-setup") {
      setCurrentSocket(
        socket_values.find((socket) => socket.value === "business-setup"),
      );
    } else if (type === "ai-workflow") {
      setCurrentSocket(
        socket_values.find((socket) => socket.value === "ai-workflow"),
      );
    } else if (!notallowUserToAccess(user, [6])) {
      setCurrentSocket(
        socket_values.find((socket) => socket.value === "general-support"),
      );
    }
  }, [type]);
  ...
  const runSocketIssue = async () => {
    ...
    const { token: sokcetToken, signature } = await getChatUtils(
      user_id,
      getCurrentSocketEncodeKey(),
    );
  ...
  const handleSaveMessage = (message) => {
    if (message.message === "Chat History") {
      const previousMessages = message?.data?.messages?.map((message) => ({
        text: message.text,
  ...
  const composeAttachmentMessage = (attachments = []) => {
    const lines = attachments.map(
      (attachment, index) =>
        `${index + 1}. ${attachment.name || `Attachment ${index + 1}`} - ${attachment.url}`,
    );
  ...
  const shouldHideAside = () => {
    ...
    if (currentPath) {
      const isAllowedIframe = allowedPaths.some((path) =>
        ...
        <>
          {!shouldHideAside() && (
            ...
            >
              ...
              {(sidebarOpen || !isMobile) && (
                <div
                  className={cn(
                    "clb-message-main",
                    sidebarOpen
                      ? "clb-message-main-sdo"
                      : "clb-message-main-sdc",
                  )}
                >
                  <ul className="clb-message-list">
                          ...
                          >
                            {type === "business-setup" && (
                              <>
                                ...
                                <p>
                                  We'll build your Business Brain step-by-step.
                                  Let's start with your team -- are you running
                                  things solo at the moment, or do you have
                            ...
                            {type === "business-overview" && (
                              <>
                                ...
                                <p>
                                  I'm Colabi-roo, your AI COO, here to help you
                                  make sense of how your business fits together.
                                </p>
                                <p>
                                  I'll take what you set up in Stage 1 -- your
                                  team, goals, plans, tools, and processes --
                                  and turn it into a clear structure you can run
                                  every day. Think of this as us laying out your
                                  business on the canvas so everything connects
                                  naturally.
                                </p>
                                <p>
                                  We'll uncover your business areas, bring in
                                  your documented processes, and shape them into
                                  simple workflow suggestions you can preview or
                                  build.
                                </p>
                                <p>
                                  Whenever you're ready, just tell me how you'd
                                  like to begin. I can walk you through it
                                  step-by-step, or you can jump straight in.
                                </p>
                              </>
                            )}
                            ...
                            {type !== "business-setup" &&
                              ...
                              type !== "ai-workflow" && (
                                <>
                                  <p>
                                    Welcome — I’m Colabi-roo, your Digital
                                    Manager, here to keep your day organised and
                                    your business running smoothly.
                                  </p>
                                  <p>From here, I can:</p>
                                  <ul>
                                    <li>
                                      Show you today’s tasks and deadlines
                                    </li>
                                    <li>Update you on workflow progress</li>
                                    <li>Check your customer activity</li>
                                    <li>
                                      Review upcoming events in your calendar
                                    </li>
                                    <li>Check your customer activity</li>
                                  </ul>
                                  <p>
                                    If this is your first time here, I can show
                                    you live data or help set up anything
                                    missing.
                                  </p>
                                </>
                              )}
                    ...
                    {voiceConversationEnabled && (
                      <li className="visualizer-container">
                        <div
                          ...
                          style={{
                            transform: `scale(${visulizerValue + 50}%)`,
                          }}
                        ></div>
                      </li>
                    )}
                    ...
                    {messages.length > 0 &&
                      ...
                      messages.map((message, index) => {
                        ...
                        let truncated =
                          idx !== -1 ? text.slice(0, idx).trimEnd() : text;
                        ...
                        return (
                          <li
                            key={index}
                            ref={
                              index === messages.length - 1 ? lastMsgRef : null
                            }
                            className={cn(
                              "clb-message-wrap",
                              message.message_type == "ai"
                                ? "clb-messageAi-wrap"
                                : "clb-messageUser-wrap",
                            )}
                          >
                            {message.message_type == "ai" && (
                              <img
                                    ...
                                    (attachment, attachmentIndex) => (
                                      ...
                                      >
                                        ...
                                        >
                                          ...
                                          <span className="clb-chat-attachmentAction">
                                            Open
                                          </span>
                                        </a>
                                      </li>
                    ...
                    {streamMessage && (
                      <li className={cn("clb-message-wrap clb-messageAi-wra")}>
                        ...
                        />
                        <div className="clb-message-box clb-messageAi-box">
                          {streamMessage}
                        </div>
                      </li>
                    )}
                    ...
                    {(loading || resLoading) && (
                      <li className="loading_chat_preview">
                        ...
                        <div
                          className={cn(
                            "clb-loader",
                            resLoading ? "clb-loading" : "clb-centered",
                          )}
                        >
                          <div className="loader-dot"></div>
                          <div className="loader-dot"></div>
                ...
                >
                  <ul className="chatFlow-btn-list">
                    ...
                    <li
                      className={cn(
                        "chatFlow-btn-item",
                        selectedBusinessTemplate === "suggested" && "active",
                      )}
                    >
              ...
              <div
                className={cn(
                  "clb-aiChat-block accordian-open",
                  sidebarOpen || isMobile ? "sidebar-open" : "sidebar-closed",
                )}
              >
                <div
                  className={cn(
                    "flex flex-col w-full sm:bottom-7 sm:static grow",
                  )}
                >
                  <div className="w-full bg-white flex flex-col gap-2.5 grow">
                    {(sidebarOpen || !isMobile) && (
                      <PasteInputComponent
                        handleSubmitText={handleMessagesAnd}
                        sidebarOpen={sidebarOpen}
                        isMobile={isMobile}
                        handleShowVideo={handleShowVideo}
                      />
                    )}
                    {!sidebarOpen && isMobile && type === "business-setup" && (
                      <ReadmapMobile
