...
function ReadmapMobile({ setCurrnetPath }) {
  ...
  const canViewFreelancer = useMemo(
    () => isUserAllowAny(user, ["view-freelancer"]),
    [user],
  );
  const isViewBusinessPlan = useMemo(
    () => isUserAllowAny(user, ["view-business-plan", "create-business-plan"]),
    [user],
  );
  const isViewJobDescriptions = useMemo(
    () =>
      isUserAllowAny(user, [
  ...
  const isViewMarketingPlan = useMemo(
    () =>
      isUserAllowAny(user, ["view-marketing-plan", "manage-marketing-plan"]),
    [user],
  );
  ...
  const completeProfilePerc = useMemo(() => {
    const flags = Object.keys(completedState).filter((key) => {
      if (key === "is_business_plan_completed" && !isViewBusinessPlan)
        return false;
      if (key === "is_core_team_completed" && !canEditTeam) return false;
      if (key === "is_marketing_plan_completed" && !isViewMarketingPlan)
        return false;
      if (key === "is_job_descriptions_completed" && !isViewJobDescriptions)
        return false;
      if (key === "add_your_current_processes" && !canCaptureProcesses)
        return false;
      if (key === "is_ai_employee_completed" && !isAIWorkForceAllow)
        return false;
      if (key === "is_freelancers_completed" && !canViewFreelancer)
        return false;
  ...
  return (
    <>
      ...
      <div className="z-50 rounded-md md:hidden flex flex-col gap-2.5 h-52 p-2 grow">
            ...
            <div className={"p-0 text-sm hover:no-underline font-normal text-[#4A4E52]"} onClick={() => setOpenAccordion(openAccordion ? "" : "item-1")}>
              <div className="flex flex-col gap-1 grow">
                <div className="flex justify-between items-center">
                  <p className="text-[13px]">Brain Training: <span className="text-[#D48A73] align-middle">{completeProfilePerc}%</span> Completed</p>
        ...
        {/* {!openAccordion ? 
          (
            <button type="button" className="bg-[#71aea3] fixed left-0 top-1/3 z-50 flex flex-col items-center px-2 py-2 rounded-2xl rounded-tl-none rounded-bl-none shadow-lg hover:shadow-xl transition-all duration-300 group" onClick={() => setOpenAccordion(openAccordion ? "" : "item-1")}>
              <ChevronLeft className="text-white" />
            </button>
          ) : 
          (
            <button type="button" onClick={() => setOpenAccordion(openAccordion ? "" : "item-1")} className="fixed left-0 top-1-4 sm:top-1/3 z-50 flex flex-col items-center bg-white p-1.5 sm:p-2 rounded-2xl rounded-tl-none rounded-bl-none shadow-lg hover:shadow-xl transition-all duration-300 group">
              <img src="/images/colabi-roo.png" alt="COLABI-ROO" className="h-14 sm:h-12 w-8 sm:w-auto object-contain mb-1" />
              <span className="text-[8px] sm:text-[10px] text-[#9CA3AF] font-medium tracking-wide">COLABI-ROO</span>
              <b className="text-[9px] sm:text-[11px] text-[#F88B62] font-semibold group-hover:text-[#E6784F] transition">Chat Here</b>
            </button>
          )
        } */}
        ...
        <div className="flex flex-col gap-2.5 h-10 grow overflow-auto p-1.5 -m-1.5 p-4">
          <ul className="bg-white border border-solid border-[#E5E7EB] shadow-[0px_1px_2px_0px_#0000000D,0px_4px_4px_0px_#00000040] rounded-md p-2 py-4 flex flex-col gap-6">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[#8B959F] text-xs font-normal -tracking-wider">
                Add the people and digital workers that help run your business.
              </p>
            </div>
            {canEditTeam && (
              <li className="flex flex-row gap-1.5 pl-2.5">
                ...
                <div className="flex flex-col gap-1">
                  <h4 className="cursor-pointer font-semibold text-[#1E1E23] text-sm leading-none pt-2.5">
                    Add Internal Team
                  </h4>
                  <p className="text-[#8B959F] text-xs font-normal -tracking-wider">
                    Core employees & team members
                  </p>
                </div>
                <div className="flex items-center gap-1 w-fit ml-auto">
                  <Button
                    className={
                      "text-[13px] font-normal leading-tight bg-gradient-to-tl from-[#1BA0A5] to-[#F98A63] text-white h-6 py-1.5 !px-2 opacity-85 hover:opacity-100"
                    }
                    onClick={() =>
                      handleCreateLink(
                        `${getOriginUrl()}/teams?reactframe=true`,
                      )
                    }
                  >
                    Add
              ...
              <li className="flex flex-row gap-1.5 pl-2.5">
                ...
                <div className="flex items-center gap-1 w-fit ml-auto">
                  <Button
                    ...
                    onClick={() =>
                      handleCreateLink(
                        `${getOriginUrl()}/guests?reactframe=true`,
                      )
                    }
                  >
                    Add
              ...
              <li className="flex flex-row gap-1.5 pl-2.5">
                ...
                <div className="flex items-center gap-1 w-fit ml-auto">
                  <Button
                    ...
                    onClick={() =>
                      handleCreateLink(
                        `${getOriginUrl()}/my-freelancer-index?reactframe=true`,
                      )
                    }
                  >
                    Add
              ...
              <li className="flex flex-row gap-1.5 pl-2.5">
                ...
                <div className="flex items-center gap-1 w-fit ml-auto">
                  <Button
                    ...
                    onClick={() =>
                      handleCreateLink(
                        `${getOriginUrl()}/workforce?reactframe=true`,
                      )
                    }
                  >
                    Add
          ...
          <ul className="bg-white border border-solid border-[#E5E7EB] shadow-[0px_1px_2px_0px_#0000000D,0px_4px_4px_0px_#00000040] rounded-md p-2 py-4 flex flex-col gap-6">
            <div className="flex flex-col gap-1">
              <span className="text-[#1E1E23] font-semibold text-base col-span-full">
                ...
                <span className="text-[#9698A0] font-semibold text-xs">
                  (Optional)
                </span>
              </span>
              <p className="text-[#8B959F] text-xs font-normal -tracking-wider">
                Strategic documents that define your business direction.
              </p>
            </div>
            {isViewBusinessPlan && (
              <li className="flex flex-row gap-1.5 pl-2.5">
                ...
                <div className="w-1/4 grow flex flex-col gap-1">
                  <h4 className="cursor-pointer font-semibold text-[#1E1E23] text-sm leading-none  -tracking-wider">
                    Add Business Plan
                  </h4>
                  <p className="font-normal text-[#9698A0] text-xs -tracking-wide">
            ...
            {isViewMarketingPlan && (
              <li className="flex flex-row gap-1.5 pl-2.5">
                <div className="size-8 shrink-0 bg-[#ff8B63]/20 rounded-full p-1.5">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    className="size-full block text-[#ff8B63]"
                  >
                    <path
                      fillRule="evenodd"
                ...
                <div className="flex items-center gap-1 w-fit ml-auto">
                  <Button
                    onClick={() =>
                      handleCreateLink(
                        user?.company_hash &&
                          isUserAllow(user, "manage-marketing-plan")
              ...
              <li className="flex flex-row gap-1.5 pl-2.5">
                ...
                <div className="flex items-center gap-1 w-fit ml-auto">
                  <Button
                    onClick={() =>
                      handleCreateLink(
                        `${getOriginUrl()}/job-description/index?reactframe=true`,
                      )
                    }
                    className={
                      "text-[13px] font-normal leading-tight bg-gradient-to-tl from-[#1BA0A5] to-[#F98A63] text-white h-6 py-1.5 !px-2 opacity-85 hover:opacity-100"
              ...
              <li className="flex flex-row gap-1.5 pl-2.5">
                ...
                <div className="flex items-center gap-1 w-fit ml-auto">
                  <Button
                    onClick={() =>
                      handleCreateLink(
                        `${getOriginUrl()}/process?reactframe=true`,
                      )
                    }
                    className={
                      "text-[13px] font-normal leading-tight bg-gradient-to-tl from-[#1BA0A5] to-[#F98A63] text-white h-6 py-1.5 !px-2 opacity-85 hover:opacity-100"
        ...
        {/* <div className="border-t-2 border-solid border-[#E7E9ED] rounded-t-2xl p-1.5 text-center flex flex-col items-center gap-1">
          <h5 className="text-[#1E1E23] text-[13px] font-medium -tracking-wide text-center">Stage 1 of 3</h5>
          <ul className="flex flex-wrap items-end justify-center gap-3">
            <li className={cn("p-1.5 w-fit -tracking-wider font-semibold text-[11px] border-b-2 border-solid border-[#E5E7EB] text-[#9CA3AF]")}>Setup</li>
            <span className="h-0.5 w-3 bg-[#E5E7EB] shrink-0 mb-3.5"></span>
            <li className={cn("p-1.5 w-fit -tracking-wider font-semibold text-[11px] border-b-2 border-solid border-[#E5E7EB] text-[#9CA3AF]")}>Overview</li>
            <span className="h-0.5 w-3 bg-[#E5E7EB] shrink-0 mb-3.5"></span>
            <li className={cn("p-1.5 w-fit -tracking-wider font-semibold text-[11px] border-b-2 border-solid border-[#E5E7EB] text-[#9CA3AF]")}>Dashboard</li>
          </ul>
        </div> */}
